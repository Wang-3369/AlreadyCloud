            ...
            fetch('/search', {
                ...
                .then(data => {
                    ...
                    if (data.message) {
                        ...
                        tbody.innerHTML = "<tr><td colspan='5'>沒有找到相關課程</td></tr>";
                    } else {
                        // 存儲所有課程資料的陣列
                        const courses = [];
                            ...
                            return fetch('/mapsearch', {
                                ...
                                .then(mapData => {
                                    ...
                                    if (loggedIn){
                                        ...
                                        <td>${item['開課單位']}</td>
                                        <td>${classroomLink}</td>
                                        <td><button class="upload-btn" onclick="uploadCourseData('${item['課程名稱']}', '${item['課號']}', '${item['節數']}', '${item['星期']}', '${item['教室']}')">上傳</button></td>`;
                                        //console.log(row.innerHTML); // 查看生成的 HTML
                                    }
                                    else{row.innerHTML = `
                                        <td>${item['節數']}</td>
                                        <td>${item['星期']}</td>
